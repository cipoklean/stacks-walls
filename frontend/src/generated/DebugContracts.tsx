...
import { useGuestbook_PostMessage } from './hooks';



import { useGuestbook_GetPostByAuthor } from './hooks';



import { useGuestbook_GetPostCount } from './hooks';
...
export default function DebugContracts() {
  ...
  const [openContract, setOpenContract] = useState<string | null>("guestbook");
    ...
    <div style={S.page}>
      ...
      <div style={S.panel}>
        <button 
          style={S.panelBtn} 
          onClick={() => {
            setOpenContract(openContract === "guestbook" ? null : "guestbook");
        ...
        >
          <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
            ...
            <span style={S.panelName}>Guestbook</span>
            <span style={S.panelExt}>.clar</span>
          </div>
          <span style={S.headerTag}>{openContract === "guestbook" ? '▲' : '▼'}</span>
        </button>

        {openContract === "guestbook" && (
          <div style={Object.assign({ padding: '20px', width:'100%' })} className='space-y-4'>
                ...
                {activeTab === 'write' && (
                  <FunctionCard_Guestbook_PostMessage 
                    num="00"
                    isOpen={openFunction === "guestbook_post-message"}
                    onToggle={() => setOpenFunction(openFunction === "guestbook_post-message" ? null : "guestbook_post-message")}
                  />
                )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Guestbook_GetPostByAuthor 
      num="01"
      isOpen={openFunction === "guestbook_get-post-by-author"}
      onToggle={() => setOpenFunction(openFunction === "guestbook_get-post-by-author" ? null : "guestbook_get-post-by-author")}
    />
  )}
  ...
  {activeTab === 'read' && (
    <FunctionCard_Guestbook_GetPostCount 
      num="02"
      isOpen={openFunction === "guestbook_get-post-count"}
      onToggle={() => setOpenFunction(openFunction === "guestbook_get-post-count" ? null : "guestbook_get-post-count")}
    />
  )}
...
function FunctionCard_Guestbook_PostMessage({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useGuestbook_PostMessage();
    ...
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          ...
          <span style={S.fnName}>POST-MESSAGE</span>
        </div>
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['content'] ?? '', {"string-ascii":{"length":200}}, 'content'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>content <span style={S.argType}>STRING-ASCII</span></label>
              
              <input style={S.argInput} value={args['content']} onChange={e => setArg('content', e.target.value)} />
...
function FunctionCard_Guestbook_GetPostByAuthor({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useGuestbook_GetPostByAuthor();
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-POST-BY-AUTHOR</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
      ...
      <div style={Object.assign({}, S.formContainer, { maxHeight: isOpen ? '1000px' : '0px', opacity: isOpen ? 1 : 0, marginTop: isOpen ? '20px' : '0px' })}>
        <form style={S.form} onSubmit={async (e) => { e.preventDefault(); await call([toClarityValue(args['author'] ?? '', "principal", 'author'),]); }}>
          
            
            <div>
              <label style={S.argLabel}>author <span style={S.argType}>PRINCIPAL</span></label>
              
              <input style={S.argInput} value={args['author']} onChange={e => setArg('author', e.target.value)} />
...
function FunctionCard_Guestbook_GetPostCount({ num, isOpen, onToggle }: { num: string, isOpen: boolean, onToggle: () => void }) {
  const { data, loading, txid, txStatus, txStatusError, explorerUrl, call } = useGuestbook_GetPostCount();
  const isReadOnly = true;
  ...
  return (
    <div style={Object.assign({}, S.card, { border: isOpen ? '1px solid #FF550E' : S.card.border })}>
      <div style={Object.assign({ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' })} onClick={onToggle}>
        <div style={Object.assign({ display: 'flex', alignItems: 'center' })}>
          <span style={S.fnNumber}>{num}</span>
          <span style={S.fnName}>GET-POST-COUNT</span>
        </div>
        <div style={isReadOnly ? S.badgeRead : S.badgeWrite}>
          <div style={S.badgeDot} />
          {isReadOnly ? 'READ' : 'WRITE'}
