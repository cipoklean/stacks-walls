...
export function useGuestbook_PostMessage() {
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      ...
      const fn = (contracts as any).guestbook_postMessage;
...
export function useGuestbook_GetPostByAuthor() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).guestbook_getPostByAuthor;
...
export function useGuestbook_GetPostCount() {
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<Error | null>(null);
  const [txid, setTxid] = useState<string | null>(null);
  ...
  const call = useCallback(async (functionArgs: ClarityValue[] = []) => {
    ...
    try {
      // Use the generated contract function from the index
      const fn = (contracts as any).guestbook_getPostCount;
